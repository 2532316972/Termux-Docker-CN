...
> ⏳ 首次启动可能需要较长时间，请耐心等待，直到看到 QEMU 控制台输出或登录提示符。
...
在执行 `~/alpine/startqemu.sh` 后，当前 Termux 窗口将直接进入虚拟机文本控制台，看到类似提示：
...
- 用户名：`root`
- 密码：`MyAlpine@2025!`（输入时无回显，输完直接回车）
- 即可完成登录。
...
> ⚠️ **首次登录前，请先通过“方式一”进入虚拟机，执行以下一行命令开启 SSH 密码登录：**
...
✅ 执行完成后，在 **Termux 主程序**（非虚拟机内）新开一个窗口，执行：

```bash
ssh -p 2222 root@localhost
```

输入密码即可登录。后续均可直接使用此方式。
