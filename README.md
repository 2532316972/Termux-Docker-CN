...
2. **💪 性能优化默认配置**  
   启动脚本 `startqemu.sh` 中默认分配了**更多 CPU 核心与内存**（默认 2 核 + 1GB），适合运行轻量级容器服务。若设备性能有限，可自行编辑调整。
...
安装过程全自动，无需干预，大约需要 10～30 分钟（取决于网络和设备性能）。
...
> ✅ 登录成功后，你将进入一个完整、轻量的 Alpine Linux 系统，Docker 已预装并可直接使用！
...
> 💡 **小贴士**：无论使用哪种方式登录，操作的都是同一个虚拟机系统，数据完全同步。

---

## 🐳 如何使用 Docker？

在虚拟机内，你可以像在普通 Linux 服务器上一样使用 Docker，如果不会建议前往B大去静修。
